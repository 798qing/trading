...
PYTHONPATH=src .venv/bin/python -m cli --sample-progress --days 30 # 查看阶段3采样进度/防空转状态
...
src/
  ...
  ops/       health.py  history.py  sample_progress.py
tests/
bot.py / main.py
```
...
- Hermes quick commands：`/btc` 默认 LLM 综合解读卡、`/btcq` 快报、`/btcr` 强制刷新、`/btcl` LLM 综合解读、`/btch` 健康检查、`/btchist` 最近流水、`/btcs` 7日统计、`/btcw` 30日权重建议、`/btcsp` 30日采样进度。
- Stage 3 auto-weight：`cli --auto-weight` 只输出建议；样本少于 30 条已结算信号时只告警不改权重。
- Stage 3 full-analysis：`/btc`/`/btcl` 显式走 `--llm`；本地 `cli` 默认仍走快路径。LLM 不可用时卡片保留纯检测器结论。
